...
public:
    explicit Tester(size_t M = 512, size_t N = 2048, size_t K = 1024, size_t warmup_iterations = 1,
        ...
        //HGEMM_CHECK(m_B);
        //read file 1 to get the row and column data to set for m_B_for_sparse
        //rowcol value code (mycode)
        FILE *fptr;
        char data_string[100];
        int rowval = 0;
        int colval = 0;
        fptr = fopen("num_data.txt", "r");
        fgets(data_string, 100, fptr);
        data_string[strcspn(data_string, "\n")] = '\0';
        rowval = atof(data_string);
        printf("rowval\n");
        fgets(data_string, 100, fptr);
        data_string[strcspn(data_string, "\n")] = '\0';
        colval = atof(data_string);
        printf("colval\n");
        printf("%d %d\n", rowval, colval);
        fclose(fptr);
        //end of my code
        m_B_for_sparse = new Matrix(m_A_sparse->getCol(), (size_t) colval, "Matrix B for sparse");
        //Read a file
        //My code starts here
        //https://www.w3schools.com/c/c_files_read.php and https://www.geeksforgeeks.org/c/c-program-to-read-contents-of-whole-file/
        m_B_for_sparse->moveToHost();
        half* B = m_B_for_sparse->getHostPtr();
        //half* B_data = (half*) calloc(sizeof(half), row*col);
        fptr = fopen("float_data.txt", "r");
        float fnum = 0.0;
        int i = 0;
        while(fgets(data_string, 100, fptr)) {
            //https://www.geeksforgeeks.org/dsa/removing-trailing-newline-character-from-fgets-input/
            data_string[strcspn(data_string, "\n")] = '\0';
            B[i] = (half) atof(data_string);
            //printf("%f\n", __half2float(B_data[i]));
            i = i + 1;
        }
        fclose(fptr);

        //for(size_t i = 0; i < row*col; i = i + 1){
            //printf("%f\n", __half2float(B[i]));
            //B[i] = B_data[i];
            //printf("%f\n", __half2float(B[i]));
        //}
        m_B_for_sparse->moveToDevice();
        //Get the half pointer to m_B
        //then iterate through the values assigning them first from string to float then float to half
        //HGEMM_CHECK(m_B_for_sparse);

        m_C = new Matrix(m_M, m_N, "Matrix C");
        //HGEMM_CHECK(m_C);
        //use the data to get the column number for m_C_sparse
        m_C_for_sparse = new Matrix(m_A_sparse->getRow(), (size_t) colval, "Matrix C for sparse");
        ...
        //HGEMM_CHECK(m_base);
        m_base_for_sparse = new Matrix(m_A_sparse->getRow(), (size_t) colval, "Matrix Base");
...
private:
    ...
    void profileSparse2(Func &&hgemm, const std::string &name) {
        ...
        //My code here
        m_C_for_sparse->moveToHost();
        half* C = m_C_for_sparse->getHostPtr();
        size_t val = m_C_for_sparse->getElemNum();
        //for (size_t i = 0; i < val; i = i + 1){
            //printf("item number %d\n", i);
            //printf("of %d\n", val);
            //printf("%d\n", C[i]);
        //}
        //size_t val1 = m_C_for_sparse->getRow();
        //size_t val2 = m_C_for_sparse->getCol();
        //printf("%d\n", val1);
        //printf("%d\n", val2);
        //m_B_for_sparse->moveToHost();
        //size_t val3 = m_B_for_sparse->getRow();
        //size_t val4 = m_B_for_sparse->getCol();
        //printf("%d\n", val3);
        //printf("%d\n", val4);
        //m_A_sparse->moveToHost();
        //size_t val5 = m_A_sparse->getRow();
       // size_t val6 = m_A_sparse->getCol();
        //printf("%d\n", val5);
        //printf("%d\n", val6);
        //https://www.w3schools.com/c/c_files_write.php
        FILE *fptr;
        fptr = fopen("C_vals.txt", "w");
        for (size_t i = 0; i < val-1; i = i + 1){
            fprintf(fptr, "%d\n", C[i]);
        }
        fprintf(fptr, "%d", C[val-1]);
        fclose(fptr);
        //end of my code
        //FILE* fout;
        //fout = fopen("results_smat.csv", "a");
