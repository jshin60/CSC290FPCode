...
int main(int argc, char *argv[]) {
    GFLAGS_NAMESPACE::ParseCommandLineFlags(&argc, &argv, true);
     ...
     bool FLAGS_use_gpu = true;
     int FLAGS_gpu_rank = 0;

    omp_set_num_threads(FLAGS_cpu_procs);
    cudaSetDevice(0);

    cudaDeviceProp dev_prop;
    cudaGetDeviceProperties(&dev_prop, FLAGS_gpu_rank);
    ...
    int clockRate_khz, memClockRate_khz;
     cudaDeviceGetAttribute(&clockRate_khz, cudaDevAttrClockRate, 0);
     cudaDeviceGetAttribute(&memClockRate_khz, cudaDevAttrMemoryClockRate, 0);
    HLOG("GPU max clock rate: %.0f MHz (%0.2f GHz)", static_cast<double>(clockRate_khz) * 1e-3,
         static_cast<double>(clockRate_khz) * 1e-6);
    HLOG("Memory clock rate: %.0f MHz (%0.2f GHz)", static_cast<double>(memClockRate_khz) * 1e-3,
         static_cast<double>(memClockRate_khz) * 1e-6);
